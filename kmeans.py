...
def euclidean_distance(point1, point2):
    ...
    lst_differences = []
    # Get the difference between data points and sum them to get distance
    for i in range(len(point1)):
        get_difference = (point1[i] - point2[i]) ** 2
        lst_differences.append(get_difference)

    get_distance = sum(lst_differences)

    euclidean_distance = math.sqrt(get_distance)
...
def update_assignment(list_of_points, centroids_dict):
    ...
    for point in list_of_points:
        ...
        assignments[closest_centroid_name].append(point)

    # Store centroids in new dict
    assigned_centroids = {}

    # Get centroids that are assigned a data point
    for centroid_name, centroid_points in assignments.items():
        if centroid_points:
            assigned_centroids[centroid_name] = centroid_points
...
def mean_of_points(list_of_points):
    ...
    group_points = len(list_of_points)
    value_dimensions = len(list_of_points[0])
    get_dimensions = [0.0] * value_dimensions

    # Calculate the sum of dimensions across data points
    for point in list_of_points:
        for i in range(value_dimensions):
            get_dimensions[i] = get_dimensions[i] + point[i]

    # Create list to store mean of data points
    mean_of_points = []
    for sum_dim in get_dimensions:
        mean_of_dim = sum_dim / group_points
        mean_of_points.append(mean_of_dim)
...
def update_centroids(assignment_dict):
    ...
    updated_centroids = {}
    # Get mean of data points and update dict
    for centroid_name, centroid_points in assignment_dict.items():
        mean_cluster = mean_of_points(centroid_points)
        updated_centroids[centroid_name] = mean_cluster
